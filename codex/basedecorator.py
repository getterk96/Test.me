...
def login_required(func):
    actual_decorator = user_passes_test(
        lambda u: u.is_authenticated and u.status == User_profile.NORMAL
    )
...
def player_required(func):
    actual_decorator = user_passes_test(
        lambda u: u.is_authenticated and u.user_type == User_profile.PLAYER and
                  u.status == User_profile.NORMAL
    )
...
def organizer_required(func):
    actual_decorator = user_passes_test(
        lambda u: u.is_authenticated and u.user_type == User_profile.ORGANIZER and
                  u.status == User_profile.NORMAL
    )
...
def admin_required(func):
    actual_decorator = user_passes_test(
        lambda u: u.is_authenticated and u.user_type == User_profile.ADMINISTRATOR and
                  u.status == User_profile.NORMAL
    )
